...
**Step 4, go over your intial array comparing the elements with your pivot and organizing them in lesser for being `(<= pivot 7)` and greater for being `(> pivot 7)`, respectively.**

You can include the element equal to pivot in either lesser or greater as you wish.

Now, we have two additional arrays, lesser and greater all filled up with the appropiate elements, still not sorted though. You can free the orginal arr if you want now, we won't be needing it.
...
That is choose a pivot `3` and `9` respectively, removing them from the parent list, creating sub-arrays and adding the elements to them appropiately.
...
For `pivot = 6` we have the sub-arrays, `lesser: [4]` and `greater: []`. What we do is we join the array lesser + pivot + greater to recieve a sorted parent sub-array.
...
The catch is that this still inefficient, because this in not in-place sort, i.e. we are wasting space by creating additional arrays to make the sub-arrays, which eats time (going over and filling the new arrays up).
...
If the conditions are not satisfied that is, `i >= pivot` and `j <= pivot` we swap those two, that is because a element which should be on the right of the pivot was found by `i` on the left we swapped it with a converse but similiar element `j` found.

We terminate this process when `i >= j` that is `i` crosses `j`. By that point all elements less than pivot are on the right and more than pivot are on the left. 
...
Yes yes calm your horse down, I will give the reasoning here. That is over all of this, the pivot might not be at the correct position that is its final position yet, the example I took did take the 7 to its actual position but that's just a concidence.
...
That's a serious improvement for the Hoare Partition case. 

That's because a pre-sorted array is the worst case, O(n<sup>2</sup>), also called a *pathologically* input (yep, that's not something medical here) for quicksort.
