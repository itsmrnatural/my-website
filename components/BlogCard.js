  ...
  return (
    <div className="bg-white/5 hover:bg-white/10 rounded-lg overflow-hidden relative transition-all duration-300 hover:scale-[1.02] border border-white/5 hover:border-white/20 group h-80">
        ...
        {blog.tags && blog.tags.length > 0 && (
          <div className="absolute top-2 left-2 flex flex-wrap gap-1">
            {blog.tags.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 bg-blue-500/90 text-white text-xs rounded-full font-medium"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="p-4 flex flex-col h-40">
        <h3 className="text-white text-lg font-semibold mb-2 line-clamp-2 group-hover:text-blue-300 transition-colors">
          {blog.title}
        </h3>
        ...
        <div className="mt-2 pt-2 border-t border-white/10">
          <span className="text-blue-400 text-xs font-medium group-hover:text-blue-300 transition-colors inline-flex items-center gap-1">
            Read more
