...
export default function GpgKeyButton() {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-r from-blue-500/10 to-slate-500/10 border border-blue-500/30 rounded-lg p-4 backdrop-blur-sm"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400">
            Key ID: <span className="text-blue-300 font-mono">{gpgKeyId}</span>
          </p>
        </div>
        <button
          onClick={handleCopy}
          className="flex-shrink-0 px-3 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-all text-xs font-medium flex items-center gap-2"
        >
