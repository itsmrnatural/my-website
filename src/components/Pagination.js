...
const Pagination = ({ currentPage, totalPages, handlePreviousPage, handleNextPage }) => {
  return (
    <div className="flex justify-between items-center w-full mt-4 text-sm">
      <button
        className="bg-coffee-200 dark:bg-white/5 hover:bg-coffee-300 dark:hover:bg-white/10 border border-coffee-300 dark:border-transparent hover:border-coffee-400 dark:hover:border-white text-coffee-900 dark:text-white text-xs px-3 py-1.5 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        onClick={handlePreviousPage}
        disabled={currentPage === 1}
      >
        ← Back
      </button>
      <p className="text-coffee-900 dark:text-white font-medium text-xs md:text-sm">
